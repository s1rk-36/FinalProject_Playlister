    ...
    */
    initEditToolbarHandlers() {
        ...
        document.getElementById("close-button").onmousedown = (event) => {
            ...
            //this.model.disableToolbar();
        }
        //HANDLER FOR ADDING SONGS
        document.getElementById("add-song-button").onmousedown = (event) => {
            //this.model.addNewSong();
            this.model.add_AddSongTransaction(this.model.getSong(this.model.getPlaylistSize() - 1));
        }
    }
    ...
    */
    initModalHandlers() {
        ...
        deleteListConfirmButton.onclick = (event) => {
            ...
            this.model.view.disableToolbar();
        }
    ...
    */
    registerItemHandlers() {
        ...
        for (let i = 0; i < this.model.getPlaylistSize(); i++) {
            ...
            var dialog = document.getElementById("edit-song");
            dialog.style.display = "block";

            document.getElementById("newTitle").value = this.model.getSong(i).title;
            document.getElementById("newArtist").value = this.model.getSong(i).artist;
            document.getElementById("newYouTubeID").value = this.model.getSong(i).youTubeId;
            ...
            editSongConfirmButton.onclick = (event) => {
                ...
                var newYouTubeID = document.getElementById("newYouTubeID").value;

                // this.model.getSong(i).title = newTitle;
                // this.model.getSong(i).artist = newArtist;
                // this.model.getSong(i).youTubeId = newYouTubeID;

                let newSong = {title: newTitle, artist: newArtist, youTubeId: newYouTubeID}

                this.model.addEditSongTransaction(i, newSong, oldSong);
                dialog.style.display = "none";
                this.model.toggleConfirmDialogOpen();
                //this.model.loadListEdit(this.model.getCurrentPlaylist());
            ...
            document.getElementById("delete-song-" + i).onmousedown = (event) => {
                ...
                let oldSong = this.model.getSong(i);
                let songToDelete = document.getElementById("delete-song-span");
                songToDelete.innerHTML = "";
                songToDelete.appendChild(document.createTextNode(this.model.getSong(i).title));
                let deleteSongModal = document.getElementById("delete-song-modal");
                // OPEN UP THE DIALOG
                deleteSongModal.classList.add("is-visible");

                let removeSongCancel = document.getElementById("delete-song-cancel-button");
                removeSongCancel.onmousedown = (event) => {
                    this.model.toggleConfirmDialogOpen();
                    deleteSongModal.classList.remove("is-visible");
                }

                let removeSongConfirm = document.getElementById("delete-song-confirm-button");
                removeSongConfirm.onmousedown = (event) => {
                    this.model.toggleConfirmDialogOpen();
                    this.model.addRemoveSongTransaction(i, oldSong) ;
                    deleteSongModal.classList.remove("is-visible");
                }
            }
