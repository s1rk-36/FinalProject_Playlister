    ...
    */
    refreshPlaylist(playlist) {
        ...
        for (let i = 0; i < playlist.songs.length; i++) {
            ...
            deleteButton.setAttribute("value", "🗑");
            deleteButton.style.blockSize = "50px";
            deleteButton.style.fontSize = "30px";
            deleteButton.style.righ = "50px";
 

            let elementDiv = document.createElement("a");
            link = link + song.youTubeId;
            elementDiv.href = link;

            // PUT THE CONTENT INTO THE CARD
            let itemText = document.createTextNode(song.title + " by " + song.artist);
            let numSong = document.createTextNode((i + 1) + ". ");
            itemDiv.appendChild(numSong);
            
            elementDiv.appendChild(itemText);
